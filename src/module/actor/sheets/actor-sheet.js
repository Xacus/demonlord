export class DemonlordActorSheet extends ActorSheet {
  ...
  activateListeners(html) {
    ...
    html.find('.spell-edit').click((ev) => {
      ...
      const item = this.actor.getEmbeddedDocument('Item', liSpell.data('itemId'));
    ...
    html.find('.rest-char').click((ev) => {
      // Talents
      const talents = this.actor.getEmbeddedCollection('').filter((e) => e.type === 'talent');
      ...
      for (const spell of spells) {
        const item = duplicate(this.actor.items.get(spell._id));
    ...
    switch (itemData.type) {
      case 'armor':
        if (this.actor.data.type === 'creature') {
          ui.notifications.error(game.i18n.localize('DL.DialogWarningCreatureArmor'));
