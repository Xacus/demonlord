...
export default class DLPathSheet extends DLBaseItemSheet {
  ...
  activateListeners(html) {
    ...
    html.find('.add-level').click((ev) => {
      ev.preventDefault();
      this.item.update({ 'data.levels': [...(this.item.data.data.levels || []), new PathLevel()] });
    });
  ...
  _deleteItem(ev) {
    ...
    const itemLevel = parent.parentElement.parentElement.getAttribute('data-level');
    const itemGroup = parent.parentElement.parentElement.getAttribute('data-group');
    const itemIndex = parent.parentElement.getAttribute('data-item-id');
    const itemData = duplicate(this.item.data);
  ...
  async _updateObject(event, formData) {
    const name = formData['name'] || this.object.name;
    ...
    return this.object.update({ name: name, data: updateData });
  }
