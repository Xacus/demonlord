...
export class DemonlordItem extends Item {
  ...
  // Workaround for editing embedded documents
  async _embedActiveEffectsOwned() {
    ...
    // Search add the _id of already present effects (that have the same name and source)
    // to the effectDataList for updating the actor's effect list with the new values
    effectDataList.forEach((currenValue, index, array) => {
      for (let embeddedEffect of actor.effects)
        if (embeddedEffect.data.label === array[index].label && embeddedEffect.data.origin === array[index].origin){
          array[index]._id = embeddedEffect.data._id
          break
        }
    })
    await actor.updateEmbeddedDocuments('ActiveEffect', effectDataList, {diff: false})

    /* Remove all effects embedded in the actor generated by this item and then add the new updated effects
    console.log("DL | ACTIVE EFFECTS | EMBED ACTIVE OWNED 1",actor.effects)
    const deleted = await DLActiveEffects.removeEffectsByOrigin(actor, this)
    console.log("DL | ACTIVE EFFECTS | EMBED ACTIVE OWNED 2",actor.effects, deleted)
    await actor.createEmbeddedDocuments('ActiveEffect', effectDataList)
    console.log("DL | ACTIVE EFFECTS | EMBED ACTIVE OWNED 3",actor.effects)
   */
  }

  async _embedActiveEffectsUnowned() {
    ...
    await this.createEmbeddedDocuments('ActiveEffect', effectDataList)
  }
