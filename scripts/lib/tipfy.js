class Tipfy {
  ...
  build () {
    ...
    try {
      _html = this.tag.hasAttribute('data-tipfy-text')
        ? _tip
        : document.querySelector(_tip).outerHTML
    } catch (e) {
      _html = _tip
    }
    _html = _html
    ...
    document.body.insertAdjacentHTML(
      'beforeend',
      `<div class="tipfy" role="tooltip">${_html}</div>`
    )
    if (this.tag.hasAttribute('data-tipfy-class')) {
      document
        .querySelector('.tipfy')
        .classList.add(this.tag.dataset.tipfyClass)
    }
    this.side()
  }
  ...
  side () {
    ...
    if (!_side) {
      ...
      if (window.innerWidth < 1024) {
        ...
        _tipfy.style.left =
          _rect.x + (_rect.width - _tipfy.clientWidth) / 2 + _left + 'px'
        _tipfy.style.top =
          (_yTop ? _rect.y + _rect.height : _rect.y - _tipfy.clientHeight) +
          _top +
          'px'
      } else {
        ...
        _tipfy.classList.add(`tipfy--${_yTop ? 'top' : 'bottom'}`)
        _tipfy.style.left =
          (_xLeft ? _rect.x : _rect.x + _rect.width - _tipfy.clientWidth) +
          _left +
          'px'
        _tipfy.style.top =
          (_yTop ? _rect.y + _rect.height : _rect.y - _tipfy.clientHeight) +
      ...
      const position = {
        ...
        top: () =>
          _tipfy.setAttribute(
            'style',
            `left: ${
              _rect.x + (_rect.width - _tipfy.clientWidth) / 2 + _left
            }px;top: ${_rect.y - _tipfy.clientHeight + _top}px;`
          ),
        bottom: () =>
          _tipfy.setAttribute(
            'style',
            `left: ${
              _rect.x + (_rect.width - _tipfy.clientWidth) / 2 + _left
            }px;top: ${_rect.y + _rect.height + _top}px;`
          )
      }
      position[_side]()
